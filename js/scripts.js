...
function validateData(data, interval) {
  // Remove header and footer
  data.splice(0, 1);
  data.splice(data.length - 1, 1);

  // Check num of columns
  for (var i = 0; i < data.length; i++) if (data[i].length !== 2) return false;

  var subgroups = [];
  for (var i = 0; i < data.length; i++) subgroups.push(data[i][0]);
  var counts = {};
  subgroups.forEach(function(x) {
    counts[x] = (counts[x] || 0) + 1;
  });
  var checker = true;
  for (var key in counts) {
    if (counts[key] !== interval) checker = false;
  }
  return checker;
}
...
function processData() {
  if (!document.getElementById('miu0').value) {
    ...
    reader.onload = function() {
      ...
      if (!validateData(CSVToArray(_data), _interval)) {
